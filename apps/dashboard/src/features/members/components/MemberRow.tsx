    ...
    <tr className="border-b border-border/10 hover:bg-[#140030]/20 transition-colors group">
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-base font-bold text-white group-hover:text-cyan-400 truncate">{member.username}</span>
            <span className="text-base text-[#8e7aab] font-medium truncate">{member.displayName}</span>
          </div>
        </div>
      </td>
      <td className="px-6 py-4 whitespace-nowrap">
        <div
          ...
          className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-[#0c0020] border border-border/20 text-base font-mono text-[#a855f7] cursor-pointer hover:border-cyan-500/50 hover:text-cyan-300 transition-colors"
        >
      ...
      <td className="px-6 py-4">
        <div className="flex flex-wrap gap-1.5 max-w-[200px]">
          {member.roles.map((r, idx) => (
            <span key={idx} className="text-base font-bold px-2 py-0.5 rounded-full border" style={{ backgroundColor: `${r.color}15`, color: r.color, borderColor: `${r.color}35` }}>
              {r.name}
            </span>
          ))}
        </div>
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="flex items-center gap-3 w-40">
          ...
          <span className="text-base font-extrabold text-[#dcb6fb] whitespace-nowrap">Niv. {member.level}</span>
        </div>
      </td>
      <td className="px-6 py-4 whitespace-nowrap text-base text-[#dcb6fb]/80 font-medium">{member.joinedAt}</td>
      <td className="px-6 py-4 whitespace-nowrap text-right text-base">
        <button onClick={() => alert("Gestion de " + member.username)} className="px-3 py-1 rounded bg-[#3c096c] hover:bg-[#7b2cbf] text-white text-base font-bold transition-colors cursor-pointer border border-[#9d4edd]/30">
          Gérer
        </button>
      </td>
    </tr>
